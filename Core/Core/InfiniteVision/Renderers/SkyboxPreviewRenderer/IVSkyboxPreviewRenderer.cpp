#include "IVSkyboxPreviewRenderer.h"
...
namespace OE1Core
{
	namespace Renderer
	{
		IVSkyboxPreviewRenderer::IVSkyboxPreviewRenderer()
		{
			m_VirtualRenderMesh = AssetManager::GetGeometryI(DynamicAssetType::SPHERE);
			s_LocalShader = ShaderManager::GetShader(ShaderID::SKY_DOM_PREVIEW);
		}
		IVSkyboxPreviewRenderer::~IVSkyboxPreviewRenderer()
		{
		...
		void IVSkyboxPreviewRenderer::Render(GLuint _src_sky_dom_texture, GLuint& _dest_preview_texture, Scene* _scene)
		{
			// Create, setup, and position the camera
			std::string PREVIEW_CAMERA_TAG = "SkydomPreivewCam";
			uint64_t uid = 909090;
			...
			glBindTexture(GL_TEXTURE_CUBE_MAP, _src_sky_dom_texture);
