...
namespace OE1Core
{
	Ray::Ray(Component::CameraComponent* _camera)
	{
		m_Camera = _camera;
	}
	Ray::~Ray()
	...
	void Ray::SwapCamera(Component::CameraComponent* _camera) 
	{
		if (!m_Camera)
			return;
	...
	{
		glm::vec2 res = m_Camera->GetResolution();
	...
	{
		glm::vec4 eye_coord = glm::inverse(m_Camera->m_Projection) * _clip_coord;
		return glm::vec4(eye_coord.x, eye_coord.y, -1.0f, 0.0f);
	}
	glm::vec3 Ray::ToWorldCoord(glm::vec4 _eye_coord)
	{
		glm::vec4 world = glm::inverse(m_Camera->m_View) * _eye_coord;
