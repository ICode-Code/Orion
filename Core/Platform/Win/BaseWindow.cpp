...
namespace OE1Core
{
	BaseWindow::BaseWindow(SDL_Event* _event, const char* _name, SDL_GLContext _shared_context, int _width, int _height)
	{
		m_Event = _event;
		m_Args.UserDataKey = _name;
		m_Args.Name = _name;
		m_Args.Width = _width;
		...
		if (_shared_context)
		{
			m_Args.MainContext = _shared_context;
			m_SharedContext = _shared_context;
	...
	void BaseWindow::InitOpenGLContext()
	{
	...
	void BaseWindow::InitRenderState()
	{
		// Depth
	...
	void BaseWindow::RegisterFollowUpEventCallback(const EventCallback& _callback)
	{
		m_FollowUpEventCallback = _callback;
	}
	void BaseWindow::RegisterEventCallback(const EventCallback& _callback)
	{
		m_Callback = _callback;
	}
	void BaseWindow::RegisterImGuiSDLEventProcessorCallback(const EventPollCallback& _event_pull_callback)
	{
		m_EventPullCallback = _event_pull_callback;
	}
	void BaseWindow::RegisterFrameSizeUpdateCallback(const FrameSizeUpdateCallback& _evet_frame_size)
	{
		m_FrameSizeUpdateCallbak = _evet_frame_size;
	}
	SDL_Window* BaseWindow::GetWin()
	{
		return m_Args.Win;
	}
	...
	void BaseWindow::RegisterEventCalls()
	{
		/// Window Resize
		...
		SDL_SetEventFilter(
			...
			{
				BaseWindow* window_handler = static_cast<BaseWindow*>(user_data);
				if (!window_handler->GetWin())
					printf("Wrong\n");

				
				if (_event->type == SDL_QUIT)
				{
					EventCallback& _callback = *(EventCallback*)SDL_GetWindowData(window_handler->GetWin(), window_handler->m_Args.UserDataKey);
				...
				else if (_event->type == SDL_WINDOWEVENT)
				{
					...
					else if (_event->window.event == SDL_WINDOWEVENT_MINIMIZED)
					{
						EventCallback& _callback = *(EventCallback*)SDL_GetWindowData(window_handler->GetWin(), window_handler->m_Args.UserDataKey);
	...
	SDL_Event* BaseWindow::GetEvent() { return m_Event; }
	void BaseWindow::Update()
	{

	}
	void BaseWindow::SetClearColor(float _color[4])
	{
		if (_color != nullptr)
		{
	...
	void BaseWindow::SetClearColor(std::initializer_list<float> _list)
	{
		if (_list.size() != 4)
		{
	...
	void BaseWindow::PullEvent()
	{
		while (SDL_PollEvent(m_Event))
		{
			if(m_EventPullCallback)
				m_EventPullCallback(m_Event);
		}
